...
EOPHP
    ...
    echo >&2 "===== Prepare config.xml for MODx ========="
...
EOF
        echo >&2 "===== Change file permittion for setup/config.xml ========="
		chown www-data:www-data setup/config.xml
        echo >&2 "===== Copy DB connetcion conf. file ========="
		pwd
		#rm -rf setup/
		ls -ltr
		#chmod +x /var/www/html/config.inc.php
		echo >&2 "===== Copy core/config.inc.php file ========="
		cp /config.inc.php core/config/config.inc.php
		chown www-data:www-data core/config/config.inc.php
		chmod 777 core/config/config.inc.php
        sudo -u www-data php setup/index.php --installmode=upgrade --config=/var/www/html/setup/config.xml
  else
        echo >&2 "===== Check whether to upgrade of MODx ========="
...
EOPHP
	...
	echo >&2 "===== Finish preparation for MODx ========="
	fi
echo >&2 "===== Exit entrypoint ========="
fi
